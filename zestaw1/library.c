...
pointer_arr* create_pointer_arr(int operation_count){
    ...
    pointer_arr->operation_count = operation_count;
    pointer_arr->free_idx = 0;
    pointer_arr->operation_blocks = calloc(operation_count, sizeof(operation_block*));
    return pointer_arr;
}
...
void delete_operation(pointer_arr* pointer_arr, int block_idx, int operation_idx){
    ...
    for(int i = operation_idx; i < operation_block->difference_count - 1 && operation_block->operations[i]!=NULL; i++){
...
void delete_operation_block(pointer_arr* pointer_arr, int block_idx){
    ...
    if(block_idx>=pointer_arr->operation_count){
    ...
    for(int i = block_idx; i < pointer_arr->operation_count && pointer_arr->operation_blocks[i]!=NULL; i++){
        pointer_arr->operation_blocks[i]=pointer_arr->operation_blocks[i];
    }
    pointer_arr->free_idx--;
}
